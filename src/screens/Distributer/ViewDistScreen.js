...
const ViewDistScreen = ({navigation}) => {
  const {state} = useContext(AuthContext)
  const name = navigation.getParam('name')
  const branches = navigation.getParam('branch')
  const areas = navigation.getParam('areas')
  const beats = navigation.getParam('beats')
  const mobile = navigation.getParam('mobile')
  const status = navigation.getParam('status')
  const gstin = navigation.getParam('gstin')
  const pan = navigation.getParam('pan')
...
//   const [curr,setCurr] = useState('')
  ...
  const check1 = async() => {
    try{
      ...
      const res = await Api.put('users/updateDistributor',{id:item._id,is_active:ab})
    ...
    return(
      <>
        ...
        <View style={{flex:1,backgroundColor:'rgb(255,255,255)'}}>
          <Header title="VIEW DISTRIBUTOR" back="Dist" />
          ...
          <View style={{paddingBottom:65}}>
            ...
            <Image
              ...
              />
              <View>
              <Text style={{fontSize:25,fontFamily:'Mulish-Black', alignSelf:'center'}}>{name}</Text>
              <Text></Text>
              ...
              <View style={styles.card1}>
              <Text style={{fontSize:16,fontWeight:'bold',color:'#696969',left:28,borderBottomWidth:2,padding:10,borderBottomColor:'#e6e6e6'}}>{gstin}</Text>
              <Iconn name='money' style={{fontSize:16,fontWeight:'bold',position:'absolute', left:5, top:13,color:'#696969',backgroundColor:'#f7f7f7',padding:10,borderRadius:50}} />
              </View>
              <View style={styles.card1}>
              <Text style={{fontSize:16,fontWeight:'bold',color:'#696969',left:28,borderBottomWidth:2,padding:10,borderBottomColor:'#e6e6e6'}}>{pan}</Text>
              <Iconn name='id-card' style={{fontSize:16,fontWeight:'bold',position:'absolute', left:5, top:13,color:'#696969',backgroundColor:'#f7f7f7',padding:10,borderRadius:50}} />
              </View>
              <View style={styles.card1}>
              <Text style={{fontSize:16,fontWeight:'bold',color:'#696969',left:28,borderBottomWidth:2,padding:10,borderBottomColor:'#e6e6e6'}}>{branches}</Text>
