...
const DistScrenn = ({navigation}) => {
    ...
    const apply = () => {
      if(status){
        ...
        const results = orders.filter(person =>{
          return person.is_active === status
       }
      )
      setNewf(results)
      }else{
  ...
  useEffect(()=> {
    (async()=> {
      try{
        setLoading(true)
        const res = await Api.get('users/distributor')
        const r = await AsyncStorage.getItem('profile')
  ...
  if(!orders){
    ...
    return (
        ...
        <View style={{flex:1,backgroundColor:'#f7f7f7'}}>
          <Header title="Distributor" bar={bar} />
          <BottomSheet visible={visible} >
                ...
                <View style={{}}>
                  ...
                  <TouchableOpacity style={{position:'absolute',right:10,top:20}} ><Iconss name="close" size={20} style={{color:'black'}} onPress={()=> setVisible(!visible)} /></TouchableOpacity>
                    ...
                    />
                    <ScrollView>
                    <View style={{marginTop:50, width:'90%', alignSelf:'center',left:8,marginBottom:10}}>
                    <Text style={{marginBottom:5,color:'black'}}>Filter by branch</Text>
                    <ScrollView>
                    <MultiSelect
                hideTags
                single={true}
            ...
            renderItem= {({ item })=>{
                return (
                  <View style={{paddingLeft:20,paddingRight:20,paddingTop:1,paddingBottom:1}}>
                    {item[role]._id===profile? <TouchableOpacity onPress={()=> navigation.navigate('View',{name:item.name,branch:item.branch.branch,areas:item.areas,beats:item.beats,status:item.is_active,mobile:item.contact1,id:item._id,pan:item.pan,gstin:item.gstin,item:item})}>
                  ...
                  <Text  style={{ fontSize:16,color:'black',position:'absolute',left:140, fontWeight:'bold',top:72,color:'rgb(195,195,195)' }}>{item.contact1}</Text>
