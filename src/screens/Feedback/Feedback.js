      ...
      <View style={{ flex: 1, backgroundColor: 'rgb(255,255,255)' }}>
        ...
        <ScrollView>
          {!feed.length > 0 && !loading
            ? <View style={{ display: 'flex', justifyContent: 'center', alignSelf: 'center', marginTop: 80 }}>
              <Avatar.Image
                source={{
                  uri: 'https://i.ibb.co/qRzZQBw/undraw-No-data-re-kwbl.png'
                }}
                size={200}
              />
              <Text style={{ textAlign: 'center', fontSize: 20, padding: 10 }}>No Feedback Yet...</Text>
            </View>
            : null}
          <FlatList
            style={{ marginTop: 20 }}
            ...
            renderItem={({ item }) => {
              return (
                <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('Viewf', { item: item })}>
                  <View style={{}}>
                    <Text style={{ fontSize: 18, fontFamily: 'Mulish-Black', color: 'black' }}>{item.subject}</Text>
                    ...
                    {
                      item.is_active === 'Open'
                        ? <Text numberOfLines={3} style={[styles.status, { backgroundColor: 'lightgreen', }]}>{item.is_active}</Text>
                        : <Text numberOfLines={3} style={[styles.status, { backgroundColor: 'orange', }]}>{item.is_active}</Text>
                    }
                  </View>
                </TouchableOpacity>
              )
