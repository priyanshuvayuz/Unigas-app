...
const AddOrder = ({navigation}) => {
  ...
  const validateName1 = (text) => {
    ...
    const add= async() => {
      ...
      try{
        setLoading(true)
        const res = await Api.post('order/create',{order_id,amount,filled_cylinders:parseInt(filled_cylinders),empty_cylinders:parseInt(empty_cylinders),location,branch:bid,distributor:profile,executive:eid,customer:select1[0],cylinder_name:select2[0],manager:mid,manager_approval:'Pending',createdBy:"distributor"})
        setLoading(false)
    ...
    const magi = async() => {
        // mid= await AsyncStorage.getItem('mid')
        // eid= await AsyncStorage.getItem('eid')
        // bid= await AsyncStorage.getItem('bid')
        // nid= await AsyncStorage.getItem('name')
        // did= await AsyncStorage.getItem('profile')
        await get()
        // await get()
        // ok()
    }
    ...
    //     })
    // },[branch1])
    console.log(eid,mid,bid,profile)
    const make = async () => {
        try{
